...
Application: wrap any seeded hidden actor (a conspirator cell, a roaming monster hunter)
in its own life story (`months = { 6 18 }`) driving that actor's scheme attempts, with the is_ai kill-switch.
...
Application: an inherited blood-feud story (`var:feud_target`/`feud_originator`, transferred
to the heir on owner death), or a hired-protector story that waits for the attack it guards against.
...
give visible-odds gambles for free. Application: formal honor duels with visible odds;
give the decline branch real teeth (prestige loss, a coward reputation modifier).
...
adopt that budgeting discipline verbatim. Example application: any trait-XP rank ladder
(a knightly or scholarly order) with successor hand-off via a weighted `random_relation` pick.
...
a ward-ready-for-knighting alert; bounty/edict feed entries with combine_into_one.
...
handles discovery spread and blackmail interactions for free. Application: a `secret_heretic_circle`
secret whose on_expose alerts the faith head, with on_owner_death transferring to an heir
or fellow member.
...
time-amortization. Application: a secret society's yearly edict/bounty pass over
its bounded member list with a cycle counter.
