...
Use [ck3-gui](../../ck3-gui/SKILL.md) for the authoring workflow. The examples include historical findings from vanilla and Workshop mods. Check current local schemas and callers before adapting them. Historical layout measurements are retained here; external calibration files mentioned in older versions are not required and may no longer be available.

Keep the game installation read-only. For an isolated layout probe, copy the native gui/debug/test_gui.gui into a separate test mod at the same relative path. Do not put calibration files in a shipping mod folder.
...
1. **Scripted widgets (use this).** Register a NEW `.gui` file's top-level widget into the running
   ...
   avoids whole-file conflicts; unique registry, widget, and type names are still needed. Vanilla's `gui/scripted_widgets/_scripted_widgets.info`
...
4. **Overriding a whole vanilla `.gui` file (avoid).** Copying `hud.gui`/`window_character.gui`
   forks a multi-thousand-line file, needs comparison after patches, and hard-conflicts with other mods.
...
The following rules were recorded in an earlier four-batch calibration campaign. The evidence files are not bundled. Treat them as historical findings and reproduce the relevant case on the installed build when exact layout behavior matters:
...
Use an installed GUI preview if available for layout iteration. It does not prove live bindings, datamodels, animations, or script execution.

1. Use the project's established test UI or a separate enabled test mod. Copy the native test window into that mod, preserve its test_window name, and edit only the copy.
2. In a debug session, open it with `gui.CreateWidget gui/debug/test_gui.gui test_window`. The native component library at `gui/debug/window_component_library.gui` shows available widgets and callers.
3. Edit the test-mod copy, reload where supported, and reopen the window. Verify the changed screen actually renders. Close it with `gui.clearwidgets test_window` when finished.
4. Inspect a fresh capture yourself when an authorized capture tool is available. Otherwise ask for the specific visual observation needed. Read fresh gui_warnings.log and error.log after reproduction.

Use fresh dump_data_types output for unfamiliar bindings. For a full runtime session, follow [ck3-playtest](../../ck3-playtest/SKILL.md). If the game is unavailable, state which visual and behavior checks remain unrun.
